...
```{r xcell_heatmap, fig.width=12, fig.height=10}
library(gplots)
...
group <- c(rep("Tumour", 10), rep("Normal", 10))
group_cols <- c(Tumour = "blue", Normal = "brown")
ColSideColors <- unname(group_cols[group])
...
```{r}
## Tumour/Normal labels (must match your sample order)
group <- c(rep("Tumour", 10), rep("Normal", 10))

## Extract GSVA values for significant pathways
DEpwys_es <- brca_es[match(DEpwys, rownames(brca_es)), ]

## Colour mapping
colorLegend <- c(Tumour = "darkred", Normal = "darkblue")

## Side colours for each sample (length = ncol(DEpwys_es))
cancer_types_all.color <- unname(colorLegend[group])
names(cancer_types_all.color) <- colnames(DEpwys_es)

## Clustering
sampleClustering <- hclust(as.dist(1 - cor(DEpwys_es, method = "spearman")), method = "complete")
geneSetClustering <- hclust(as.dist(1 - cor(t(DEpwys_es), method = "pearson")), method = "complete")

## Heatmap
heatmap(
  DEpwys_es,
  ColSideColors = cancer_types_all.color,
  xlab = "samples",
  ylab = "Pathways",
  margins = c(2, 20),
  labCol = "",
...
legend("topleft", legend = names(colorLegend), fill = colorLegend, inset = 0.01, bg = "white")
