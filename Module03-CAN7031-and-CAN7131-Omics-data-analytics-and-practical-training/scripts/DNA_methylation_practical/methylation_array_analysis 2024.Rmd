...
Install packages from BiocManager::install("") in case they are not installed. For example
...
You may also need to install these packages using BiocManager::install("")
```{r}
...
Install "methylationArrayAnalysis" package now
```{r}
BiocManager::install("methylationArrayAnalysis")
```

Now, you can load all the required libraries
```{r}
library(knitr)
library(limma)
library(minfi)
